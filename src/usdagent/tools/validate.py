...
from pxr import UsdUtils  # type: ignore[import]

from usdagent.schemas import Issue, ValidationReport
from usdagent.tools.stage import get_stage


def validate_stage(stage_path: str) -> ValidationReport:
    """Run a full integrity check on the stage and return a ValidationReport with all issues."""
    issues: list[Issue] = []
    broken = find_broken_references(stage_path)

    for ref_path in broken:
        issues.append(Issue(severity="error", path=ref_path, message="Broken reference"))

    layer_issues = check_layer_stack(stage_path)
...
def find_broken_references(stage_path: str) -> list[str]:
    """Return a list of asset paths that cannot be resolved in the current stage."""
    broken: list[str] = []
    try:
        result = UsdUtils.ComputeAllDependencies(stage_path)
...
def check_layer_stack(stage_path: str) -> list[Issue]:
